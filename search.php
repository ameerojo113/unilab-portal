...
<body>
    ...
    <div class="container my-3">
        ...
        <div class="row">
            ...
            $noResult = true;
            $query = $_GET["search"];
            $sql = "SELECT * FROM `division` WHERE MATCH(divisionName, divisionDesc) against('$query')";
            ...
            ?><script>
                    ...
                    <div class="card" style="width: 18rem;">
                        <img src="img/card-' . $catId . '.jpg?" class="card-img-top" alt="image for this drugs" width="249px" height="270px">
                        <div class="card-body">
                            <h5 class="card-title"><a href="viewdrugsList.php?catid=' . $catId . '">' . $catname . '</a></h5>
                            <p class="card-text">' . substr($catdesc, 0, 29) . '...</p>
                            <a href="viewdrugsList.php?catid=' . $catId . '" class="btn btn-primary">View All</a>
                        </div>
                    </div>
                </div>';
                        }
    ...
    <div class="container my-3" id="cont">
        ...
        <div class="row">
            <?php
            $query = $_GET["search"];
            $sql = "SELECT * FROM `drugs` WHERE MATCH(drugsName, drugsDesc) against('$query')";
            $result = mysqli_query($conn, $sql);
            while ($row = mysqli_fetch_assoc($result)) {
            ?><script>
                    document.getElementById("iteam").innerHTML = "Items: ";
                            ...
                            $drugsdivisionId = $row['drugsdivisionId'];

                            echo '<div class="col-lg-4 col-md-6 col-12 d-flex justify-content-center p-5 text-center">
                    <div class="card" style="width: 18rem;">
                        <img src="img/drugs-' . $drugsId . '.jpg?" class="card-img-top" alt="image for this drugs" width="249px" height="270px"\>
                        <div class="card-body">
                            <h5 class="card-title">' . substr($drugsName, 0, 20) . '...</h5>
                            <h6 style="color: #ff0000">₱ ' . $drugsPrice . '</h6>
                            ...
                            echo '</form>
                                <a href="viewdrugs.php?drugsid=' . $drugsId . '"><button class="btn btn-primary">Quick View</button></a>
                            </div>
                        </div>
                    </div>
                </div>';
